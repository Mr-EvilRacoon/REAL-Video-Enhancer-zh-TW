...
class HandleApplication:
    ...
    def listBackends(self):
        ...
        if pytorch_device:
            dev_label = pytorch_device.lower()
            if dev_label == "rocm": dev_label = "rocm"
            elif dev_label == "cuda": dev_label = "cuda"
            elif dev_label == "xpu": dev_label = "xpu"
            elif dev_label == "mps": dev_label = "mps"
            else: dev_label = "cpu" if dev_label == "cpu" else pytorch_device
            availableBackends.append(f"pytorch ({dev_label})")
            ...
            try:
                ...
                pyTorchGpus = ["CPU"]
                printMSG += f"Warning: get_gpus_torch failed: {e}\n"
        ...
        if ncnn_ver:
            ...
            try:
                ...
                ncnnGpus = ["CPU"]
                printMSG += f"Warning: get_gpus_ncnn failed: {e}\n"
            printMSG += f"NCNN Version: 20220729\n"
            for i, gpu in enumerate(ncnnGpus):
                printMSG += f"NCNN GPU {i}: {gpu}\n"
        
        if not availableBackends:
            availableBackends.append("pytorch (cpu)")
            printMSG += f"PyTorch Version: {pytorch_version or 'unknown'}\n"
            printMSG += f"PyTorch GPU 0: CPU\n"
            half_prec_supp = False
            if ncnn_ver is None:
                printMSG += "Note: NCNN not installed, CPU fallback enabled\n"
