    ...
    def handle_device(device, gpu_id: int = 0) -> torch.device:
        ...
        if device == "auto":
            if torch.cuda.is_available():
                ...
                torchdevice = torch.device("xpu", gpu_id)
            else:
                torchdevice = torch.device("cpu")
        elif device == "cuda":
            if torch.cuda.is_available():
                torchdevice = torch.device(device, gpu_id)
            elif hasattr(torch, "xpu") and torch.xpu.is_available():
                torchdevice = torch.device("xpu", gpu_id)
            else:
                torchdevice = torch.device("cpu")
            ...
            try:
                torchdevice = torch.device(device, gpu_id) if device in ("cuda","xpu") else torch.device(device)
            except Exception:
                torchdevice = torch.device("cpu")
        try:
            gpus = backendDetect.get_gpus_torch()
            name = gpus[gpu_id] if gpu_id < len(gpus) else gpus[0] if gpus else "CPU"
            print("Using Device: " + str(name), file=sys.stderr)
        except Exception:
            print("Using Device: " + str(torchdevice), file=sys.stderr)
        return torchdevice
    ...
    def clear_cache():
        try:
            if torch.cuda.is_available():
                ...
                try: torch.cuda.reset_max_memory_allocated()
                except Exception: pass
                try: torch.cuda.reset_max_memory_cached()
                except Exception: pass
            elif hasattr(torch, "xpu") and torch.xpu.is_available():
