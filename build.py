...
OUTPUT_FOLDER = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dist")
...
class PythonManager:
    ...
    PYTHON_VENV_PATH = "venv312\\Scripts\\python.exe" if PLATFORM == "win32" else "venv/bin/python3"
    ...
    def run_venv_python(cls, command: str):
        import shlex
        if PLATFORM == "win32":
            # On Windows, shlex.split handles quotes differently
            # Use a simpler approach: split by space but respect quotes
            import re
            parts = re.findall(r'(?:[^\s"]|"(?:\\.|[^"])*")+', command)
            parts = [p.strip('"') for p in parts]
        else:
            parts = shlex.split(command)
        command = [cls.PYTHON_VENV_PATH,] + parts
    ...
    def __create_venv(self):
        ...
        command = [self.PYTHON_SYSTEM_EXECUTABLE, "-m", "venv", "venv312"]
        subprocess.run(command)
...
class BuildManager:
    ...
    def build_gui(self):
        ...
        if PLATFORM == "win32":
            os.system(
                r".\venv312\Lib\site-packages\PySide6\uic.exe -g python testRVEInterface.ui > mainwindow.py"
            )
    ...
    def build_resources(self):
        ...
        if PLATFORM == "win32":
            os.system(
                r".\venv312\Lib\site-packages\PySide6\rcc.exe -g python resources.qrc > resources_rc.py"
            )
...
class PyInstaller(BuildManager):
    ...
    def build(self):
        ...
        cmd = [
            ...
            "--hidden-import", "src.ModelHandler",
            "--hidden-import", "src.Backendhandler",
            "--hidden-import", "src.version",
            "--hidden-import", "src.VideoInfo",
            "--hidden-import", "src.DownloadDeps",
            "--hidden-import", "src.DownloadModels",
            "--hidden-import", "src.GenerateFFMpegCommand",
            "--hidden-import", "src.DiscordRPC",
            "--hidden-import", "src.BuiltInTorchVersions",
            "--hidden-import", "src.ui",
            "--distpath", OUTPUT_FOLDER,
        ]
        PythonManager.run_venv_python(" ".join(f'"{a}"' if " " in a else a for a in cmd))
...
if __name__ == "__main__":
    ...
    if not os.path.exists("venv312") or not args.build == "gui":
